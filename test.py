    ...
    while step < n_steps and episode < n_episodes:
        ...
        while not (terminated or truncated):
            ...
            if discrete:
                actions = int(
                    discrete_actions[0]
                )  # [int(discrete_actions[0]), int(dac[0])]
            else:
            ...
            if (terminated or truncated) and env.render_mode:
                env.display_board(env.board)
                print(f"ep: {episode}, r: {ep_reward}")
        # print(aloss, closs)
        ...
        if episode % er == 0 and episode > 1:
            ...
            if episode % (er * 5) == 0:
                ...
                env = TTTNvN(1, 1, "human", op_type="human", obs_as_array=True)
                print(env.render_mode)
                env.__dict__["close"] = __close
        if episode % er == 1 and episode > 1:
            print("no more human")
            env.close()
            env = TTTNvN(1, 1, "", op_type="random", obs_as_array=True)
            env.__dict__["close"] = __close
...
if __name__ == "__main__":
    ...
    discrete_env = TTTNvN(1, 1, "", True, True)
